...
import pageNames from '../registerPage/pageName';

// 主菜单name list
const TabBarList = [pageNames.Home, pageNames.My];
...
const navigationUtil = {
   ...
   * @param pageName
   */
  pushSingleScreenApp: (pageName, init = false) => {
    // 仅在初始化时设置默认配置
    init && navigationUtil.getTabBarConfig();
    Navigation.setRoot({
      root: {
        stack: {
          children: [{
            component: {
              ...
              options: {
                topBar: {
                  title: {
                    text: pageName === pageNames.Login ? '' : pageName
                  },
                }
              },
            }
